...
- Use `scripts/pipeline_live_view.py --refresh-seconds 5` for a continuously refreshing single-pane terminal view (`--view-mode rotate` for small terminals, `--view-mode full` for all sections per frame; add `--no-alt-screen` if needed)
