...
- Prefer `bash scripts/shard_offload_cycle.sh` in automation (pre-reconcile + symlink rehydrate -> gated offload -> post-reconcile -> enforce-hot-manifests) instead of direct one-shot offload calls
